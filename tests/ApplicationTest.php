    ...
    testCase('using the real file system', function () {
        ...
        testCase('runs the install:assets command', function () {
            setUpBeforeClass(function () {
                ...
                $command = $application->find('install:assets');
        ...
        testCase('runs the install:assets command specifying the argument "then-package-file"', function () {
            setUpBeforeClass(function () {
                $application = static::getVar('application');
                $command = $application->find('install:assets');
                $arguments = [
                    'directory' => TEMP_DIR2.'/examples',
                    'then-package-file' => TEMP_DIR2.'/then-package.json',
                ];
    ...
    testCase('using a virtual file system', function () {
        testCase('exists a directory', function () {
            ...
            testCase('exists a composer.lock file that contains two then packages', function () {
                ...
                test('the command "list:packages" shows the expected packages', function () {
